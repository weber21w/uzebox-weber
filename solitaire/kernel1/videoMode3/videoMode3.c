...
/*
void 2bppPalletBlit(unsigned char spriteNo,unsigned char ramTileNo,unsigned int xy,unsigned int dxdy){
   ...
   u8 sno = sprites[spriteNo].tileIndex%4;
...
*/
	...
	void PalletBlitSprite(u8 sprNo,u8 ramTileIndex,u16 yx,u16 dydx){
		u8 dy=dydx>>8;
		u8 dx=dydx &0xff;
		...
		u8 rbuf[64];
		u8 riter;
		u16 src=(sprites[sprNo].tileIndex*16)//(sprites[sprNo].tileIndex*TILE_HEIGHT*TILE_WIDTH)
				+sprites_tile_banks[flags>>6];	//add bank adress		
		
		for(x2=0;x2<64;x2+=4){
			px = pgm_read_byte(src++);
			if(px&0b11000000)
				rbuf[x2+0] = (px&0b11000000)>>6;
			if(px&0b00110000)
				rbuf[x2+1] = (px&0b00110000)>>4;
			if(px&0b00001100)
				rbuf[x2+2] = (px&0b00001100)>>2;
			if(px&0b00000011)
				rbuf[x2+3] = (px&0b00000011)>>0;  
		}
		riter = 0;
		...
		for(y2=0;y2<(TILE_HEIGHT-ydiff);y2++){
			for(x2=0;x2<(TILE_WIDTH-destXdiff);x2++){
				...
				px=rbuf[riter];//pgm_read_byte(src);
