...
;if(x == 0){	
	...
	mov r17,r18	;r17 = dx
;   if(SPRITE_FLIP_X){
	sbrs r16,SPRITE_FLIP_X_BIT;if(SPRITE_FLIP_X
	rjmp x_check_end;else

	adiw ZL,(TILE_WIDTH-1);src += 7
	ldi r17,16
	sub r17,r18	;16-dx xdiff for src = 16-xdiff
	rjmp x_check_end
;}else{//x == 1
...
x_2nd_tile:
	...
	rjmp x2_flip_x
;if(!SPRITE_FLIP_X){
	mov r17,r24	;xdiff for src x-dx
	add ZL,r24;src += 8-xdiff
	adc ZH,r1	
	rjmp x_check_end
;}else{
x2_flip_x:
...
*/
	...
	clt;clear T flag
	sbrc r16,SPRITE_FLIP_X_BIT;if !SPRITE_FLIP_X(skip if bit in register cleared)
	set;set T flag in S reg if not SPRITE_FLIP_X
...
y2_loop:
	...
	brts x2_loop_flip;branch if T flag set(if not SPRITE_FLIP_X
...
x2_loop_flip:
	lpm r18,Z;r18 = px = pgm_read_byte(src)
	sbiw ZL,1;src--
	cpse r18,r19;if(px != COLOR_KEY
	st X,r18;*dest = px;
	adiw XL,1;dest++
	dec r20;loopcount--
	brne x2_loop_flip
