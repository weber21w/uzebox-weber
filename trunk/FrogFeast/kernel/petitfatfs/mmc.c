...
WORD async_last_message;
/* Exchange a byte */
void xmit_spi_async(		/* Returns received data */
...
static
BYTE rcv_spi_async(void)	/* Returns received data */
{
	SPDR = 0xff;
	loop_until_bit_is_set(SPSR, SPIF);
	return SPDR;
...
#define RES_WOULD_BLOCK 32
WORD async_attempts;
WORD async_step;
WORD async_ofs;
WORD async_max_steps = 1;
WORD async_max_spi;
WORD async_last_status;
WORD async_bc;
uint16_t async_attempts_per_frame =40000;
...
async_step++;
ASYNC_STEP_1:
	send_cmd(CMD17, lba);	// READ_SINGLE_BLOCK 
async_step++;
return RES_WOULD_BLOCK;
ASYNC_STEP_2:
//	async_attempts = 40000;
		for(uint16_t i=0;i<async_attempts_per_frame;i++){
		//do {							// Wait for data packet 
			rc = rcv_spi_async();
			//if(rc == RES_WOULD_BLOCK)
			//	return RES_WOULD_BLOCK;
			if(rc != 0xFF)
				break;//if(!= 0xFF)//forget error handling
		}

		if(rc != 0xFF){//good
		//} while (rc == 0xFF && async_last_status != RES_WOULD_BLOCK);// && --async_attempts);
	async_step++;
//	return RES_WOULD_BLOCK;
}
ASYNC_STEP_3:
